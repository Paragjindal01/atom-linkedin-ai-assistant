...
import { Bot, Sparkles, AlertCircle, Copy, CheckCircle2, Loader2, Link as LinkIcon, Type, Hash } from 'lucide-react';
...
const AskAtom = () => {
  ...
  const [mode, setMode] = useState('LinkedIn Post'); // 'LinkedIn Post' or 'LinkedIn Comment Reply'
  ...
  const handleGenerate = async (e) => {
    ...
    if (mode === 'LinkedIn Comment Reply') {
      payload.content_type = 'LinkedIn Comment Reply';
      payload.platform = 'LinkedIn';
      delete payload.keywords;
      delete payload.word_count;
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-full min-h-[60vh]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400 animate-pulse">Initializing Atom engine...</p>
          </div>
        </div>
      </DashboardLayout>
    );
        ...
        <div className="max-w-4xl mx-auto text-center mt-20">
          <div className="glass-panel rounded-3xl p-12 border border-amber-500/20">
            ...
            <p className="text-slate-400 mb-8 max-w-md mx-auto">
              Before Atom can generate highly-targeted content for you, it needs to learn about your business.
            </p>
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">
              Generate highly-converting, brand-aligned marketing copy instantly.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          >
            <form onSubmit={handleGenerate} className="space-y-6">
              ...
              <div className="flex bg-[#0f172a] p-1 rounded-xl mb-6 border border-white/10">
                <button
                  type="button"
                  onClick={() => setMode('LinkedIn Post')}
                  className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                    mode === 'LinkedIn Post' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  LinkedIn Post
                </button>
                <button
                  type="button"
                  onClick={() => setMode('LinkedIn Comment Reply')}
                  className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                    mode === 'LinkedIn Comment Reply' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Comment Reply
                </button>
              ...
              <div className="p-4 bg-white/[0.02] border border-white/10 rounded-2xl mb-6">
                <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-wider flex items-center gap-2">
                  <LinkIcon className="w-4 h-4 text-purple-400" /> Connection Context
                </h3>
              ...
              <div>
                <label className={labelClass}>
                  {mode === 'LinkedIn Comment Reply' ? 'Paste LinkedIn Comment / Post Text' : 'Topic / Instructions'}
                </label>
                <textarea 
                  name="topic" 
                  ...
                  placeholder={mode === 'LinkedIn Comment Reply' ? 'Paste the text you want to reply to...' : 'What should this content be about? Be specific.'} 
                  required 
                />
              </div>
                  ...
                  {generating ? (
                    ...
                    >
                      ...
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-sm">Synthesizing {formData.tone.toLowerCase()} {formData.content_type.toLowerCase()}</p>
                      </div>
                    </motion.div>
                  ) : result ? (
                    <motion.div 
                    ...
                    >
                      ...
                      <p>Fill out the parameters on the left and click "Ask Atom" to generate high-quality marketing content.</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
