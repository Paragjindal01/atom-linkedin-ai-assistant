...
const Dashboard = () => {
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-full min-h-[60vh]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400 animate-pulse">Loading Atom workspace...</p>
          </div>
        </div>
      </DashboardLayout>
    );
  ...
  if (error) {
    return (
      <DashboardLayout>
        <div className="glass-panel rounded-2xl p-8 flex flex-col items-center text-center max-w-lg mx-auto mt-20 border-red-500/20">
          <AlertCircle className="w-12 h-12 text-red-400 mb-4" />
          <h3 className="text-xl font-bold text-white mb-2">Failed to load stats</h3>
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-400">Here is what is happening with your marketing campaigns today.</p>
        </div>
        ...
        {isEmpty ? (
          <motion.div 
            ...
            className="glass-panel rounded-3xl p-12 text-center mt-12 border border-white/5"
          >
              ...
              {stats.recentCampaigns?.length > 0 ? (
                <div className="space-y-4">
                  {stats.recentCampaigns.map(campaign => (
                    <div key={campaign.id} className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-colors flex items-center justify-between">
                      <div>
                      ...
                      <span className={`px-3 py-1 text-xs rounded-full font-medium ${
                        ...
                        campaign.status === 'draft' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/20' : 
              ...
              {stats.recentGeneratedContent?.length > 0 ? (
                <div className="space-y-4">
                  {stats.recentGeneratedContent.map(content => (
                    <div key={content.id} className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-cyan-300">{content.platform}</span>
                        <span className="text-xs text-slate-400">{new Date(content.created_at).toLocaleDateString()}</span>
                      </div>
                      <p className="text-white text-sm line-clamp-2">{content.topic || content.prompt || 'Generated content'}</p>
                    </div>
                  ))}
                </div>
              ) : (
